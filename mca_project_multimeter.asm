...
//----------------conversion part for ac voltage measurement----------------
	...
	MOV A,R7				
	MOV B,#86d					;value to be multiplied to get the reading in rms
...
freq2:
	JNB	PSW.4,down9				;----changed by y2g 19jan20
		MOV	Sp,#09h				;----changed by y2g 19jan20  reset stack pointer
		MOV 09,#0F0h			;----changed by y2g 19jan20  update dummy return address on TOS
		MOV 08,#0Bh				;----changed by y2g 19jan20  update dummy return address on TOS
		RETI
		dummy_freq2_ret:
		MOV	TCON,#01h			;----changed by y2g 19jan20  clear previous INT0 and config INT0 as -ve edge triggered

down9:	MOV R0,#01h				;----changed by y2g 19jan20				
	MOV R1,#01h					;----changed by y2g 19jan20
	MOV	R2,#01h					;----changed by y2g 19jan20
	MOV R3,#00h					;----changed by y2g 19jan20
	CLR PSW.4					;back to bank 0
...
freq_calc:
	...
	CHECK1:						;checks to see whether freq is greater than 10k
	MOV	TMOD,#15h				;counter 0 mode 1 , timer 1 mode 1
	SETB P3.4
	MOV TL0, #0EFh				;COUNT UPTO 10000
	MOV	TH0, #0D8h
	SETB TR0
	MOV R0, #20
	
AGAIN:
	MOV TL1, #0B0h				;1 sec timer
	MOV TH1, #3Ch
	
	SETB TR1
BACK:
	JB TF0, next_check1			;if tf0 triggers means the freq is greater than 10k, after which the time period is reduced to 0.1 sec
	JNB TF1, BACK
	CLR TF1
	CLR TR1
	DJNZ R0,AGAIN
	...
	JMP prev					; if it is less than 10k then count and show it on the lcd directly
	
//----------------value lies between 1 and 10k----------------
	
	prev:						;counter is set to 0 and displayed upto freq of 10k
	
	;RESETING THE TIMER/COUNTER FROM THE PREVIOUS OPERTION
	CLR TR0						;STOPS THE COUNTER FROM PREVIOUS OPERATION
	CLR TF0						;CLEARS THE TF0 FROM THE PREVIOUS OPERATION
	CLR TR1
	CLR TF1
...
BACK2:
	...
	SJMP freq_calc
//----------------10k to 100k----------------
next_check1:					;if freq greater than 10k check whether less than 10k in timing of 0.1s, if overflow occurs than freq is greater than 100k
	;RESETING THE TIMER/COUNTER FROM THE PREVIOUS OPERTION
	CLR TR0						;STOPS THE COUNTER FROM PREVIOUS OPERATION
	CLR TF0						;CLEARS THE TF0 FROM THE PREVIOUS OPERATION
	CLR TR1
...
BACK3:
	JB TF0, next_check2			;if tf0 triggers means the freq is greater than 100k, after which the time period is reduced to 0.01 sec
	JNB TF1, BACK3
	CLR TF1
	CLR TR1
	DJNZ R0,AGAIN3
	...
	JMP prev2					; if it is less than 10k then count and show it on the lcd directly
	
	
//----------------value lies between 10k and 100k----------------
	prev2:						;counter is set to 0 and displayed upto freq of 10k
...
//----------------100k to 1Mhz----------------
	next_check2:				;time period changed to 0.01s 
